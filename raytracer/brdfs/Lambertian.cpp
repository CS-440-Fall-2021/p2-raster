...
Lambertian::Lambertian(const Lambertian &l) : BRDF()
{
    kd = l.kd;
    cd = l.cd;
}
...
RGBColor Lambertian::f(const ShadeInfo &, const Vector3D &, const Vector3D &) const
{
    return (kd * cd * invPI);
}

RGBColor Lambertian::sample_f(const ShadeInfo &sinfo, const Vector3D &, Vector3D &wi, float &pdf) const
{
...
RGBColor Lambertian::sample_f(const ShadeInfo &, const Vector3D &, Vector3D &) const
{
    return (kd * cd * invPI);
}
