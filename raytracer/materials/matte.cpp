...
Matte::Matte(Lambertian *ambient_brdf_, Lambertian *diffuse_brdf_) : Material()
{
    if (ambient_brdf_ == NULL)
    {
        ambient_brdf = new Lambertian();
        ambient_brdf->set_kd(0.0);
        ambient_brdf->set_cd(25);
    }
    if (diffuse_brdf_ == NULL)
    {
        diffuse_brdf = new Lambertian();
        diffuse_brdf->set_kd(0.0);
        diffuse_brdf->set_cd(75);
    }
}
