...
void BVH::build()
{
    ...
    while (stack_ptr > 0)
    {
        ...
        int dim = centroid_box.maximum_axis();
        float arr[3] = {centroid_box.pmin.x + centroid_box.pmax.x, centroid_box.pmin.y + centroid_box.pmax.y, centroid_box.pmin.z + centroid_box.pmax.z};
        float split = 0.5 * (arr[dim]);
        int mid = left;

        for (int i = left; i < right; i++)
        {
            Point3D point = geom_array[i]->getCenter();
            float point_arr[3] = {point.x, point.y, point.z};
            if (point_arr[dim] < split)
            {
...
ShadeInfo BVH::hit_objects(const Ray &ray) const
{
